...
def fetch_nse_official_data():
    ...
    while True:
        try:
            ...
            # Step 3: Fetch ALL Sectoral Indices from NSE 🔥 (FIXED)
            ...
            if idx_res.status_code == 200:
                ...
                for idx in idx_data:
                    ...
                    key_type = idx.get("key", "")
                    
                    # Nifty 50, Nifty Bank, aur saare "SECTORAL INDICES" ko utha lo
                    if index_name in ["NIFTY 50", "NIFTY BANK"] or key_type == "SECTORAL INDICES":
                        # Avoid duplicates
                        if not any(d['Index'] == index_name for d in clean_indices):
